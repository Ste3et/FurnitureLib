...
public class ItemStack_Paper_V121_5 extends ItemStackReader{

	private static Class<?> clazz_NBTReadLimiter, clazz_CraftServer, clazz_HolderLookup_a, clazz_tag;
	...
	static{
		try {
			...
			clazz_tag = Class.forName(getNbtFolder() + ".Tag");
			...
			parse = clazz_nms_item.getMethod("parse", clazz_HolderLookup_a, clazz_tag);
	...
	public ItemStack getItemStack(NBTTagCompound nbtTagCompound) {
		try {
            Optional<?> nms_item = (Optional<?>) parse.invoke(null, provider, convertCompound(nbtTagCompound));
